...
impl CellType {
    ...
    #[must_use]
    pub fn with_generic<T: Send + Sync + 'static>(mut self) -> Self {
        fn make<T_: Send + Sync + 'static>(cap: u32) -> Box<dyn GenericColumnAny> {
...
///
/// The component type itself is metadata, not a raw page column. Only its
/// declared field types need to satisfy [`crate::page::Pod`]. Requiring and
/// auto-implementing Pod for the whole struct used to make ordinary
/// inter-field padding part of a falsely safe raw-byte contract.
pub trait SceneColumnSet: 'static {
